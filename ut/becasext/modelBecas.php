<?php
	...
	class modelBecas {
		...
		function cinicial($vals) {
			$sql = "SELECT Id, Nombre, Descripcion, RutaImg, RutaDoc, Tipo, Activo, DATE_FORMAT(Creado,'%d/%m/%Y %H:%i:%s') AS Creado FROM BECAS $vals ORDER BY Creado DESC";
			...
			while($row = $this->resultados->fetch_array()) {
				...
				$salida.="
					<tr>
						...
						<td align='center'>
							<div class='btn-group'>
								<button type='button' class='btn btn-warning btn-xs fa fa-edit' onclick='editar(\"$IdBeca\",\"$Nombre\",\"$Descripcion\",\"$RutaImg?".time()."\",\"$RutaDoc?".time()."\",\"$Tipo\",\"$Activo\");' data-toggle='tooltip' title='Editar'></button>
							</div>
							<div class='btn-group'>
