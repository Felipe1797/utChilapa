...
$(function () {
	frmAgregar = $("#frmAgregar").validate({
		...
		rules: {
			...
			DescripcionAdd: { maxlength: 1000 },
			ImgAdd: { required: true ,filesize: 1048576 },
			ModilidadAdd: { required:true }
		}, messages: { 
			ImgAdd: { 
	...
	frmEditar = $("#frmEditar").validate({
		...
		rules: {
			...
			DescripcionEdit: { maxlength: 1000 },
			ImgEdit: { filesize: 1048576 },
			ModilidadEdit: { required: true }
		}, messages: { 
			ImgEdit: {
				filesize: "Imagen mayor 1Mb. ____"
			}
...
function agregar(){
	...
	ajax.open("POST", "addeditMovilidad.php");
	ajax.send(formdata);
...
function editar(idNota,nombre,descripcion,rutaImg,movilidad,activo) {
	$("#mdEditar").modal();
	$("#divImgEditPreview").show();
	$("#tfIdEdit").val(idNota);
	...
	$("#cmbMovilidadEdit").val(movilidad);
	if (activo==1) {
