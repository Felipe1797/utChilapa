...
$(function () {
	frmAgregar = $("#frmAgregar").validate({
		ignore: [],
		rules: {
			NombreAdd: { required: true, maxlength: 150 },
			NivelAdd: { required: true },
			DocAdd: { required: true },
			IdCuatrimestreAdd: { required: true, maxlength: 150 },
			IdCarreraAdd: { required: true, maxlength: 150 }
		}
	});
	...
	frmEditar = $("#frmEditar").validate({
		ignore: [],
		rules: {
			NombreEdit: { required: true, maxlength: 150 },
			IdCuatrimestreEdit: { required: true, maxlength: 150 },
			IdCarreraEdit: { required: true, maxlength: 150 }
		}
	});
...
function agregar(){
	...
	ajax.open("POST", "addeditAsignatura.php");
	ajax.send(formdata);
...
function editar(idNota,nombre,nivel, rutaDoc,idcuatrimestre,idcarrera) {
	$("#mdEditar").modal();
	$("#divImgEditPreview").show();
	$("#tfIdEdit").val(idNota);
	...
	idcuatrimestre = idcuatrimestre.replace(/&quoxyz/g, '"');
	idcuatrimestre = idcuatrimestre.replace(/&apoxyz/g, "'");
	$("#tfIdCuatrimestreEdit").val(idcuatrimestre);
