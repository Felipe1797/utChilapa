<?php
	...
	if(isset($_FILES["ImgEdit"]["type"])) {
		...
		if($_FILES["ImgEdit"]["type"] != null && $_FILES["DocEdit"]["type"] != null) {
			...
			$act = "UPDATE BECAS SET Nombre='".$Nombre."', Descripcion='".$Descripcion."', RutaImg='".$rutaImg."', RutaDoc='".$rutaDoc."', Tipo='".$Tipo."', Activo=".$Activo." WHERE Id='".$IdBeca."'";
			...
			$act = "UPDATE BECAS SET Nombre='".$Nombre."', Descripcion='".$Descripcion."', RutaImg='".$rutaImg."', Tipo='".$Tipo."', Activo=".$Activo." WHERE Id='".$IdBeca."'";
			...
			$act = "UPDATE BECAS SET Nombre='".$Nombre."', Descripcion='".$Descripcion."', RutaDoc='".$rutaDoc."', Tipo='".$Tipo."', Activo=".$Activo." WHERE Id='".$IdBeca."'";
			...
			$act = "UPDATE BECAS SET Nombre='".$Nombre."', Descripcion='".$Descripcion."', Tipo='".$Tipo."', Activo=".$Activo." WHERE Id='".$IdBeca."'";
			$tildes = $conexion->query("SET NAMES 'utf8'");
			if(@mysqli_query($conexion,$act))
			{
				$becas->bitacora('Modificación',$_SESSION['usuario_nombres']." ".$_SESSION['usuario_apellidos'],$_SESSION['usuario_cargo'],'Becas');
		...
		if ($_FILES["ImgAdd"]["type"]!=null && $_FILES["DocAdd"]["type"]!=null ) {
			...
			if ((($_FILES["ImgAdd"]["type"] == "image/png") || ($_FILES["ImgAdd"]["type"] == "image/jpg") || ($_FILES["ImgAdd"]["type"] == "image/jpeg")) && ($_FILES["ImgAdd"]["size"] < 1048576) && in_array($file_extension, $validextensions)){
				if ($_FILES["ImgAdd"]["error"] > 0) {
					...
					$act = "INSERT INTO BECAS(Id, Nombre, Descripcion, RutaImg, RutaDoc, Tipo, Activo, Creado, IdUsuario) VALUES (0,'".$Nombre."','".$Descripcion."','".$rutaImg."','".$rutaDoc."','".$Tipo."', true, NOW(),'".$IdUsuario."')";
