<?php
	...
	class modelReglamento {
		...
		function getIdReglamento() { return $this->IdReglamento; }
		...
		function setIdReglamento($IdReglamento) { $this->IdReglamento=$IdReglamento; }
		...
		function cinicial($vals) {
			$sql = "SELECT Id, Nombre, RutaDoc, Activo, DATE_FORMAT(Creado,'%d/%m/%Y %H:%i:%s') AS Creado FROM REGLAMENTOS $vals ORDER BY Creado DESC";
					...
					<tr>
						...
						<td align='center'>
							<div style='width:100px;'>
								<div class='btn-group'>
									<button type='button' class='btn btn-default btn-xs' onclick='verImg(\"$RutaDoc?".time()."\");'><i class='fa fa-eye'></i>Ver Reglamento</button>
								</div>
							</div>
						</td>
						<td>";
						...
						<td align='center'>
							<div class='btn-group'>
								<button type='button' class='btn btn-warning btn-xs fa fa-edit' onclick='editar(\"$IdReglamento\",\"$Nombre\",\"$RutaDoc?".time()."\",\"$Activo\");' data-toggle='tooltip' title='Editar'></button>
							</div>
							<div class='btn-group'>
								<button type='button' class='btn btn-danger btn-xs glyphicon glyphicon-remove' onclick='eliminar(\"$IdReglamento\", \"$RutaDoc\");' data-toggle='tooltip' title='Eliminar'></button>
							</div>
						</td>
					</tr>";
			}
		...
		function eliminar($IdNota, $RutaDoc) {
			$sqlEliminar = "DELETE FROM REGLAMENTOS WHERE Id='".$IdNota."'";
			$this->conexion->conexion->set_charset('utf8');
			...
			echo "Bien";
			$this->bitacora('Eliminación',$_SESSION['usuario_nombres']." ".$_SESSION['usuario_apellidos'],$_SESSION['usuario_cargo'],'Reglamento');
		}
