...
$(function () {
	frmAgregar = $("#frmAgregar").validate({
		...
		rules: {
			...
			TestimonioAdd: { required: true, maxlength: 1000 }
		}
	});
	...
	frmEditar = $("#frmEditar").validate({
		...
		rules: {
			...
			TestimonioEdit: { required: true, maxlength: 1000 }
		}
	});
});
...
function cinicial(Vals) {
	...
	$.ajax({
		url:"controlSeguimientoEgresados.php",
...
function agregar() {
	...
	var Testimonio = $("#tfTestimonioAdd").val();
	Testimonio = Testimonio.replace(/\r?\n/g, "<br>");
	$.ajax({
		url:"controlSeguimientoEgresados.php",
		data:"opcion=agregar&Nombre=" + Nombre + "&Testimonio=" + Testimonio,
		type:"POST"
	}).done(function(resultados) {
		if (resultados != null && resultados != "") {
			if (resultados == "Bien") {
...
function editar(idSeguimiento,nombre,testimonio,activo) {
	$("#mdEditar").modal();
	$("#divImgEditPreview").show();
	$("#tfIdEdit").val(idSeguimiento);
	nombre = nombre.replace(/&quoxyz/g, '"');
	nombre = nombre.replace(/&apoxyz/g, "'");
	$("#tfNombreEdit").val(nombre);
	testimonio = testimonio.replace(/&quoxyz/g, '"');
	testimonio = testimonio.replace(/&apoxyz/g, "'");
	testimonio = testimonio.replace(/<br>/g, "\n");
	$("#tfTestimonioEdit").html(testimonio);
	if (activo==1) {
...
function modificar() {
	var IdSeguimiento = $("#tfIdEdit").val();
	var Nombre = $("#tfNombreEdit").val();
	var Testimonio = $("#tfTestimonioEdit").val();
	Testimonio = Testimonio.replace(/\r?\n/g, "<br>");
	var Activo = $("#chkActivoEdit").prop("checked");

	$.ajax({
		url:"controlSeguimientoEgresados.php",
		data:"opcion=modificar&IdSeguimiento=" + IdSeguimiento + "&Nombre=" + Nombre + "&Testimonio=" + Testimonio 
												+ "&Activo=" + Activo,
		type:"POST"
	}).done(function(resultados) {
		if (resultados != null && resultados != "") {
